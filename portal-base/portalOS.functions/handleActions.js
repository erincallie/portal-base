...
exports.main = async (context, sendResponse) => {
  ...
  if (properties) {
    ...
    let associations = context.body.associations;
    let objectId = context.body.objectId;
    let endpoint = `https://api.hubapi.com/crm/v3/objects/${objectType}/${objectId}`;
    let update_response = await updateObject(endpoint, properties, associations, headers)
    if (picture_file || image_file) {
      let this_pic = picture_file || image_file;
        sendResponse({body: {message:this_pic}, statusCode: 200});
    }
  } else {
    let fromObjectType = context.body.fromObjectType;
    let fromObjectId = context.body.fromObjectId;
    let toObjectType = context.body.toObjectType;
    let toObjectId = context.body.toObjectId;
    let assocTypeId = context.body.assocTypeId;
